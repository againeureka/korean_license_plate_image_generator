...
class ImageGenerator:
    ...
    def type01(self, num, save=False):
        ...
        for i, Iter in enumerate(range(num)):
            ...
            bboxes.append([col, row, col + 56, row + 83])
            col += 56
            ...
            # character 3
            hangul_char = convert_keyboard_code_to_hangul(self.char_list[i % 37])
            label += hangul_char
            safe_paste(Plate, char[i % 37], row, col)
            bboxes.append([col, row, col + 60, row + 83])
            col += (60 + 36)
            ...
            save_path = self.save_path + label
            if save:
                cv2.imwrite(save_path + ".jpg", Plate)
